...
target_net.load_state_dict(policy_net.state_dict())#Loads a model’s parameter dictionary using a deserialized state_dict.
target_net.eval()
...
def resample_sql(sql_list):
    ...
    for sql in sql_list:
        ...
        for t in count():
            ...
            if done:
                ...
                print("VALEUR",np.exp(reward*log(1.5)-log(pg_cost)))
    ...
    import random
    print("**************************************************************************************************")
    #print("aaaaaaaaaaaa",rewardsP)
    res_sql = []
    #print("bbbbbbbbbbbb",mes/len(sql_list))
    print("mrcs",rewardsP)
...
def train(trainSet,validateSet):
    ...
    for i_episode in range(0,10000):
        ...
        for t in count():
            ...
            if done:
                ...
                if ((i_episode + 1)%print_every==0):
                    mean_rel_cost.append(sum(MRC)/len(MRC))
                    print("MRC",sum(MRC)/len(MRC))
                    MRC=[]
        ...
        mrc=np.exp(reward.item()*log(1.5)-log(pg_cost))
        
        MRC.append(mrc)
        
                # break
    torch.save(policy_net.cpu().state_dict(), 'CostTraining.pth')
    print("Mean Relevent Latency ", mean_rel_cost)
